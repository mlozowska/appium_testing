...
class TestCalculator:
    ...
    def test_div_by_zero(self, driver):
        self.home_page = HomePage(driver)
        self.home_page.div_numbers(5, 0)
        result = self.home_page.get_preview_result()
        assert result == "Can't divide by 0"

    def test_sqrt(self, driver):
        self.home_page = HomePage(driver)
        self.home_page.sqrt_number(9)
        result = self.home_page.get_result()
        assert result == "3"
